...
<#
.SYNOPSIS
  Build (and optionally sign) signed MSIX packages for the SSMS Command
  Palette extension.
...
.PARAMETER Version
  Three-part version (e.g. 1.0.0). The .0 revision is appended for the
...
param(
    ...
    # demo connections instead of reading the local SSMS settings file.
...
$ExtensionName = "SSMSCommandPalette"
...
$signingPfx = $null
if ($CertBase64) {
    $signingPfx = Join-Path $env:TEMP "ssmscp-signing-$([Guid]::NewGuid().ToString('N')).pfx"
